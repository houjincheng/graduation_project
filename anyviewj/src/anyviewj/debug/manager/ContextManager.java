...
public class ContextManager implements Manager {
	...
	public void deactivated(SessionEvent sevt) {
    	...
    	BasicSession se = (BasicSession)sevt.getSession();
    	if(se.getCurrentSourcePane()!=null)
    	{
    	 DebuggingHighlighter dbg = se.getCurrentSourcePane().editorPanel.bpal.dbg;
    	 if(dbg!=null)
         dbg.reDrawBP((SourceEditor)se.getCurrentSourcePane().editorPanel.editor);
    	}
        setCurrentLocation(null, false);
        //SourcePane sp = ((BasicSession)ConsoleCenter.getCurrentSession()).getCurrentSourcePane();
        //Element map = ((SourceEditor)sp.editorPanel.editor).getDocument().getDefaultRootElement();
        //((SourceEditor)sp.editorPanel.editor).lineHighlighter.setHighlight(((SourceEditor)sp.editorPanel.editor).getStyledDocument(),0,0);
    } // deactivated
