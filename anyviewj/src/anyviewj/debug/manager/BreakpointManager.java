...
import anyviewj.debug.breakpoint.ManagerGroup;
...
import anyviewj.debug.breakpoint.MethodBreakpoint;
import anyviewj.debug.breakpoint.ResolveException;
import anyviewj.debug.breakpoint.SourceNameBreakpoint;
import anyviewj.debug.breakpoint.UncaughtExceptionBreakpoint;
import anyviewj.debug.breakpoint.event.BreakpointEvent;
import anyviewj.debug.breakpoint.event.BreakpointListener;
import anyviewj.debug.breakpoint.event.GroupEvent;
import anyviewj.debug.breakpoint.event.GroupListener;
import anyviewj.debug.event.EventListenerList;
...
public class BreakpointManager implements Manager, VMEventListener {
     ...
     */
    public Hashtable breakpointsTable;
    ...
    public BreakpointManager(ConsoleCenter aCenter) {
        super();
        this.center = aCenter;
        listeners = new EventListenerList();
        breakpointsTable = new Hashtable();        
	...
	public void setSession(Session e) {  
    	System.out.println("BreakpointManager setSession");
    	this.getDefaultGroup().setSession(e);
    	owningSession = e;
	...
	public void activated(SessionEvent sevt) {
    	System.out.println("BreakpointManager activated");
       ...
       LinkedList< SourceNameBreakpoint> lfbp = new LinkedList< SourceNameBreakpoint>();
       Set key = breakpointsTable.keySet();
   	for(Object s: key)
   	{
   		Breakpoint bp = (Breakpoint)breakpointsTable.get(s);
   	 if (bp instanceof ResolvableBreakpoint)
   	 {
   		 SourceNameBreakpoint bp2 = (SourceNameBreakpoint)bp;
    	 lfbp.add(bp2);
   	 }
   	}
     
   	SourceNameBreakpoint temp = null;
    if(!lfbp.isEmpty())
    for(int i = lfbp.size()-1;i>0;i--)
    {
      for(int j = 0;j<i;j++)
      {
    	  if(lfbp.get(j+1).lineNum<lfbp.get(j).lineNum)
    	  {
    		  temp = lfbp.get(j);
    		  lfbp.set(j,lfbp.get(j+1));
    		  lfbp.set(j+1,temp);
    	  }
      }
    }	
    ...
    for(int i = 0;i<lfbp.size();i++)
    	try{
         	resolveBreakpoint(lfbp.get(i));
    	}
        	catch(Exception e){
        		e.printStackTrace();
        	}
...
//        ResolvableBreakpoint extends AbstractBreakpoint
        if (bp instanceof ResolvableBreakpoint) {
	...
	public void closing(SessionEvent sevt) {
        ...
        prefs = Preferences.userRoot().node(
            "anyviewj/bluemarsh/jswat/breakpoint/defaultGroup");
	...
	public boolean eventOccurred(Event e) {
      ...
      //  System.out.println("Recevied = "+eventRequest.hashCode()+"    "+e.toString());
...
//    } // opened
	...
	public void opened(Session session) {
    	...
    	System.out.println( "public void opened(Session session)");
        owningSession = session;

        // Recreate the breakpoints from the persistent store.
        Preferences prefs = Preferences.userRoot().node(
            "anyviewj/debuger/breakPoint/defaultGroup");
        defaultGroup = new ManagerGroup(session);
        if (!defaultGroup.readObject(prefs)) {
            // Fall back on this if the deserialization failed.
           /* session.getUIAdapter().showMessage(
    ...
    protected void listenForClassPrepareEvents() {
        if (owningSession!=null && !listeningForClassPrepare) {
            VMEventManager vmeman = (VMEventManager)
                owningSession.getManager(VMEventManager.class);
    ...
    public void setCurrentLocation(Location temp){
        ...
        if(owningSession == null){
            ...
            if (vm != null && !bp.isResolved() && bp.isEnabled()) {
            	...
            	if(currentLocation!=null)
            	{
            		if(bp instanceof ResolvableBreakpoint)
            		{
            			int current = ((SourceNameBreakpoint)bp).lineNum;
            			int locate = currentLocation.lineNumber();
            			if(locate<current)
            			{

            				bp.eventRequest = null;
//            				 fireChange(bp, BreakpointEvent.TYPE_MODIFIED);
            			}
            		}
            	}
...
//                }
    ...
    {
    	Set set = this.breakpointsTable.keySet();
    	for(Object key: set)
    	{
    		fireChange((Breakpoint)breakpointsTable.get(key),BreakpointEvent.TYPE_REMOVED);
    		//breakpointsTable.remove(key);
    	}
