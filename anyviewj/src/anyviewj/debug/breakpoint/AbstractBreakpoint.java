...
public abstract class AbstractBreakpoint
    ...
    */
          ...
          incrementStoppedCount();
           shouldResume = shouldResume(e);
            if (!shouldResume) {
                ...
                try{
                ((ResolvableBreakpoint)o).resolveLocatableEvent(e);
                getBreakpointManager().setCurrentLocation(((LocatableEvent)e).location());
                }
                catch(Exception x){
                	x.printStackTrace();
                }
                if(((ResolvableBreakpoint)o).isResolved())
                {
                    fireChange();
                }
            }
    ...
    protected BreakpointManager getBreakpointManager() {        
        BreakpointManager brkman = (BreakpointManager)ConsoleCenter.getCurrentSession().getManager(BreakpointManager.class);
        return brkman;
