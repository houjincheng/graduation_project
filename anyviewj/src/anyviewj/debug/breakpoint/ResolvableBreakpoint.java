...
public abstract class ResolvableBreakpoint extends AbstractBreakpoint {
    ...
    public transient EventRequest eventRequest;
    ...
    protected void addPrepareRequest(ClassPrepareRequest req) {
        ...
        System.out.println("Resolvable = "+req.hashCode());
    ...
    public boolean resolveLocatableEvent(Event event)
            throws ResolveException {
    	    if(event instanceof LocatableEvent)
    	    { 	
            ReferenceType clazz = ((LocatableEvent)event).location().declaringType();
            EventRequest er = null;
            if (referenceSpec.matches(clazz)) {
                er = resolveReference(clazz);
                if (er != null) {
    ...
    public void resolveEagerly(VirtualMachine vm)
        ...
        while (iter.hasNext()) {
        	/*ReferenceType 包括目标虚拟机中装载的类，接口和数组的类型（数组
        	也是一种对象，有自己的对象类型）。ReferenceType 有三种子接口：
        	ClassType 对应于加载的类，InterfaceType 对应于接口，
        	ArrayType 对应于数组。另外，ReferenceType 还提供了一组方法，
        	可以用来获取该类型中声明的所有变量、方法、静态变量的取值、内嵌类、运行实例、行号等信息。*/
