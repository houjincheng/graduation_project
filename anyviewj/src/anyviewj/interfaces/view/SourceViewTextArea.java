 ...
 * @author  ltt
 */
public class SourceViewTextArea extends JPanel implements ActionListener, Scrollable {
    ...
    public SourceViewTextArea(char[] buf) {    	
    	...
    	contentArea = new JTextArea(){
			...
			public void paint(Graphics g){	
		    	System.out.println("paint in TextArea");
		    	super.paint(g);		    	
		    	SourceViewTextArea.this.repaint();                        
		    }
		    

    	};    	
    	srcollPane = new JScrollPane(contentArea);   	       
	...
	public void paint(Graphics g) {
    	...
    	int start = contentArea.viewToModel(srcollPane.getViewport().getViewPosition()); 
    	// starting pos in document                
    	int end =  contentArea.viewToModel(new Point(
    			srcollPane.getViewport().getViewPosition().x + contentArea.getWidth(), 
    			 srcollPane.getViewport().getViewPosition().y + contentArea.getHeight()));                
    	// end pos in doc// translate offsets to lines                
    	Document doc = contentArea.getDocument();                
    	int startline = doc.getDefaultRootElement().getElementIndex(start) + 1;
    	int endline = doc.getDefaultRootElement().getElementIndex(end) + 1;
    	int fontHeight = g.getFontMetrics(contentArea.getFont()).getHeight();
    	int fontDesc = g.getFontMetrics(contentArea.getFont()).getDescent();
    	int starting_y = -1;
    	try{
    		starting_y = contentArea.modelToView(start).y 
    				          -srcollPane.getViewport().getViewPosition().y 
    				          + fontHeight - fontDesc;                
    ...
    protected void paintGutter(Graphics g) {      	
    	...
    	try{
    		starting_y = contentArea.modelToView(start).y 
    				          -srcollPane.getViewport().getViewPosition().y + 5 + radius; 
    				          //+ fontHeight - fontDesc;                
    	}catch (BadLocationException e1){
    			e1.printStackTrace();                
    	}
        ...
        while (line <= endline) {
            ...
            Color c = ctx.getBackColor();
            if(! c.equals(gutterColor)){
               g.setColor(ctx.getBackColor());
               g.fillOval( 10 - radius, y -radius, 2 * radius, 2 * radius);
    ...
    void repaintGutter() {
        ...
        if(session != null){        
        	ContextManager cm =(ContextManager)session.getManager(ContextManager.class);
        	ThreadReference rt = cm.getCurrentThread();
        	if(rt != null){
        		//System.out.println("line ="+cm.getCurrentLocation().lineNumber());
        		DefaultHighlighter h = (DefaultHighlighter)contentArea.getHighlighter();
            	class MyHighlightPainter extends DefaultHighlighter.DefaultHighlightPainter {  
                    public MyHighlightPainter(Color color) {   
                        super(color);   
                    }   
                }   
                MyHighlightPainter p = new MyHighlightPainter(Color.BLUE);   
                try {   
                   int start = contentArea.getLineStartOffset(cm.getCurrentLocation().lineNumber());   
                   int end = contentArea.getLineEndOffset(cm.getCurrentLocation().lineNumber());   
    ...
    public void setContent(SourceContent content) {
        ...
        char[] buffer = content.getBuffer();
        contentArea.setText(new String(buffer)); 
        repaint();
    } // setContent
    ...
    public int viewToLine(Point pt) {
        ...
        FontMetrics metrics = getFontMetrics(contentArea.getFont());
        int lineHeight = metrics.getHeight();
    ...
    public void paintText(Graphics g){
    	...
    	// end pos in doc// translate offsets to lines                
    	Document doc = contentArea.getDocument();                
    	int firstLine = doc.getDefaultRootElement().getElementIndex(start) + 1;
    	int lastLine = doc.getDefaultRootElement().getElementIndex(end) + 1;
    	int fontHeight = getFontMetrics(contentArea.getFont()).getHeight();    	
    	int starting_y = -1;
    	try{    		
    		starting_y = contentArea.modelToView(start).y 
    				          -srcollPane.getViewport().getViewPosition().y + 5 + radius; 
        ...
        while (line < lastLine) {
            ...
            MyHighlightPainter p = new MyHighlightPainter(Color.lightGray);  
            ...
            if (updateLength > 0) {
                ...
                Color c = ctx.getBackColor();
                System.out.println("line "+line+" color = "+c);
                if(!c.equals(getBackground())){                    
                    try {   
                    	int s = contentArea.getLineStartOffset(line);
                    	int e = contentArea.getLineEndOffset(line);                    
                        h.removeAllHighlights();   
                        h.addHighlight(s, e, p);
                        x = e;
                    }catch (BadLocationException e1) {   
                        e1.printStackTrace();   
                    }                    
